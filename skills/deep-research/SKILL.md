...
1. Topology 감지:
   - `node ${CLAUDE_PLUGIN_ROOT}/templates/topology-detector.js "$TARGET_ROOT"` 또는 동등한 Node module 호출로 topology를 감지한다.
...
3. Health Check 실행:
   - `node ${CLAUDE_PLUGIN_ROOT}/health/health-check.js "$TARGET_ROOT" --skip-audit` 또는 `runHealthCheck($TARGET_ROOT, { fitnessPath })` 동등 호출을 실행한다.
...
Agent(
  ...
  prompt="area=patterns; work_dir=<$WORK_DIR>; task=<task_description>;" +
         "target_root=<$TARGET_ROOT>; re_run_area=<--scope or null>; incremental_since=<--incremental or null>"
)
Agent(
  subagent_type="deep-work:research-{codebase|zerobase}-worker",
  model=state.model_routing.research,
