...
1. **Session resolution** (기존 deep-brainstorm과 동일):
   ...
   - 모두 실패 시: "활성 deep-work 세션이 없습니다. `$deep-work:deep-work-orchestrator`부터 시작하세요." 출력 후 종료(exit 1)

2. **State file 경로**: `.codex/deep-work.${SESSION_ID}.md`
   - 파일 부재 시: 동일 에러 메시지 + 종료
   - `current_phase` 읽기. `test` 또는 `idle`이 아니면 경고 후 선택:
     - `current_phase == "implement"` 등 → numbered-choice prompt "Phase 4 Test를 완료하지 않았습니다. 계속 진행할까요?"
     - 사용자 취소 → 종료
...
4. **Skip 분기**: `$ARGUMENTS`에 `--skip-integrate` 있으면
   ...
   - orchestrator에게 즉시 반환 → finish flow로 진행
...
2. 파일 존재 시 (재진입):
   ...
   - `null`이면 번호형 사용자 확인: "이전 Phase 5 루프가 중단되었습니다. (1) 이어서 (2) 처음부터 (3) skip"
...
두 파일의 생성 여부 확인. 실패 시 "Phase 5 시그널 수집 실패" 경고 + `integrate-loop.json`에 `terminated_by: "error"` 기록. Section 4의 종료 절차는 **스킵**하고 skill을 에러 종료시킨다 — orchestrator가 이 종료를 감지하면 `--skip-integrate`와 함께 finish flow를 호출하여 state machine을 닫는다 (v6.3.0 review C2). `phase5_completed_at`은 기록하지 않으며, `phase5_entered_at`만 남은 상태는 `--skip-integrate`가 우회한다.
...
Codex `spawn_agent` 기본 에이전트에게 다음 프롬프트를 전달한다 (signal envelope를 LLM 프롬프트로 전달하고 JSON 응답을 받는 단순 추론용이므로 특화 에이전트 불필요):
...
`numbered-choice prompt`로 선택 수집. 각 분기:
- 1/2/3 → 해당 recommendation을 실행 (Section 3-5)
- 기타 → numbered-choice prompt 자유 입력, `plugin/command/args` 직접 수집 후 실행
...
자연어 추천 3개를 단순 규칙으로 표시:
  1. deep-review Codex skill (세션에 충분한 코드 변경 있음)
  2. deep-docs Codex skill (AGENTS.md/README.md 변경 시)
  3. $deep-wiki:wiki-ingest <work_dir> (세션 규모가 중간 이상일 때)
...
1. 추천된 Codex skill invocation을 사용자에게 제시한다. 스킬이 번호형 사용자 확인으로 "이 작업을 실행하시겠습니까?"라고 확인한 뒤 사용자가 해당 `$plugin:skill` 호출을 수동 입력하는 방식이다.

   **중요 — v1 단순 경로**: 스킬은 다음 작업을 "제안"만 하고 실제 실행은 사용자의 다음 Codex skill 호출에 맡긴다. Phase 5 내부 자동 dispatch는 v1 범위 밖.
...
3. **재진입 시 직전 라운드 확인**:
   - `executed[]`에 마지막 항목이 있고 `outcome == "completed"`이면, 번호형 사용자 확인:
...
3. 사용자에게 요약 출력:
   ...
   실행 이력: <N>개
   다음: migrated `deep-finish` spec 또는 finish flow
   ```
