...
① deep-review 플러그인 설치 확인
    ...
    └─ 미설치 → ②로
        ↓
② codex / gemini CLI 설치 확인
    ├─ 하나 이상 설치됨 → 크로스 모델 리뷰 + 셀프 리뷰 + 독립 Codex 서브에이전트 리뷰 (병렬)
    └─ 둘 다 미설치 → ③으로
        ↓
③ 셀프 리뷰 + 독립 Codex 서브에이전트 리뷰 (병렬)
```
...
**deep-review 설치 확인:**
```bash
ls "$HOME/.codex/plugins/cache/"*/deep-review/.codex-plugin/plugin.json 2>/dev/null
```

설치됨: `$deep-review:deep-review-workflow` 실행 또는 Codex `spawn_agent` 리뷰어 사용
미설치: 다음 fallback으로 진행

---

## 2. 독립 Codex 서브에이전트 리뷰

Codex `spawn_agent`로 독립 리뷰 서브에이전트를 직접 스폰한다 (백그라운드).
플러그인 의존 없이, 현재 Phase 산출물과 리뷰 관점을 프롬프트로 전달한다.

**스폰 방법:**
```
spawn_agent worker message:
...
Phase별로 메인 Codex 세션이 직접 산출물을 재검토한다:
...
번호형 사용자 확인으로 항목별 수정/스킵을 선택받은 후 수정 진행.
