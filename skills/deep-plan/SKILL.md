...
M/L/XL slice에 contract 필드 필수. Codex worker/evaluator로 검증:
...
```bash
bash ${DEEP_WORK_PLUGIN_ROOT}/hooks/scripts/notify.sh "$STATE_FILE" "plan" "completed" "Plan 작성 완료" 2>/dev/null || true
```
