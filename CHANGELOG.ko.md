...
## Unreleased — Phase B + C 진행 중 (2026-04-26)

### Phase C (사람 검토 + deep-review 대응)
...
- `AskUserQuestion` structured options (`header`, `multiSelect`) — 자연어 번호 매김 prompt 로 변환
- `TeamCreate` / `TeamDelete` / `TeamGet` — B-α 미지원 (자연어 fallback)
- `SendMessage` pattern 1 (parallel aggregation) — **B-α 보존** (`spawn_agent` N + `wait` N)
- `SendMessage` pattern 2 (sequential chain + 양방향 receipt) — **deadwood** (Branch A 전체 미지원, env_var 활성 시 Branch B 로 fall-through)
...
3. 첫 `/deep-work` 실행 시 hook 자동 install prompt 응답
