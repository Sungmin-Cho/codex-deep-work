...
_acquire_lock() {
  ...
  local err_log="${PROJECT_ROOT:-$PWD}/.codex/deep-work-guard-errors.log"
...
# Manages .codex/deep-work-current-session for env var fallback.
...
# ─── Registry read/write ───────────────────────────────────
# Central registry: .codex/deep-work-sessions.json

read_registry() {
  local registry_file="$PROJECT_ROOT/.codex/deep-work-sessions.json"
...
write_registry() {
  local json="$1"
  local registry_file="$PROJECT_ROOT/.codex/deep-work-sessions.json"
  local lock_path="$PROJECT_ROOT/.codex/deep-work-sessions.lock"
...
migrate_legacy_state() {
  local legacy_file="$PROJECT_ROOT/.codex/deep-work.local.md"
  ...
  local task_desc
  task_desc="$(read_frontmatter_field <(read_state_file "deep-work.${new_id}.md") "task_description")"
...
register_fork_session() {
  ...
  local parent_state="$PROJECT_ROOT/.codex/deep-work.${parent_id}.md"
